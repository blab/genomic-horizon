...
available_analyses=['treeLength','RC','Sharp','tmrcas','transitions','subtrees','ancestry'] ## analysis names that are possible
        ...
        if treecount==0: ## At tree state 0 insert header into output file
            ...
            if lower==0 and upper==np.inf: ## only add a header if not doing a chunk
                ...
                if 'transitions' in analyses:
                    outfile.write('\ttotalChangeCount\tcompleteHistory_1')
                ###########################################
                if 'subtrees' in analyses:
                    import copy
                ###########################################
                if 'ancestry' in analyses:
                    targets=['EBOV|14859_EMLK|KU296438|SLE|Kambia|2015-04-30','EBOV|MK3462|KU296565|SLE|Bombali|2015-03-26','EBOV|EM_004422|KR817097|GIN|Kissidougou|2015-01-02','EBOV|PL5294|KU296764|SLE|Kambia|2015-03-24']
                    timeline=np.linspace(2013.5,2016.0,200)
                    locations=['Bomi', 'Bong', 'Gbarpolu', 'GrandCapeMount', 'GrandBassa', 'GrandGedeh', 'GrandKru', 'Lofa', 'Margibi', 'Maryland', 'Montserrado', 'Nimba', 'RiverCess', 'RiverGee', 'Sinoe', 'Boffa', 'Boke', 'Fria', 'Gaoual', 'Koundara', 'Conakry', 'Dabola', 'Dinguiraye', 'Faranah', 'Kissidougou', 'Kerouane', 'Kankan', 'Kouroussa', 'Mandiana', 'Siguiri', 'Coyah', 'Dubreka', 'Forecariah', 'Kindia', 'Telimele', 'Koubia', 'Lelouma', 'Labe', 'Mali', 'Tougue', 'Dalaba', 'Mamou', 'Pita', 'Beyla', 'Gueckedou', 'Lola', 'Macenta', 'Nzerekore', 'Yamou', 'Kailahun', 'Kenema', 'Kono', 'Bombali', 'Kambia', 'Koinadugu', 'PortLoko', 'Tonkolili', 'Bo', 'Bonthe', 'Moyamba', 'Pujehun', 'WesternRural', 'WesternUrban']
                    outfile.write('\t%s'%('\t'.join(['%s_%.4f'%(target,time) for target in targets for time in timeline])))
                    # sys.stderr.write('\t%s'%('\t'.join(['%s_%.4f'%(target,time) for target in targets for time in timeline])))
                ###########################################
...
#                                     outfile.write('\t%s'%(subtreeString))
            ###################################################
            if 'ancestry' in analyses:
                target_tips=sorted([k for k in ll.Objects if k.branchType=='leaf' and k.name in targets],key=lambda x:targets.index(x.name))

                ref=[k for k in ll.Objects if k.branchType=='leaf' and k.name=='EBOV|DML24825|KT357849|SLE|Tonkolili|2015-02-04'][-1]
                # print ll.treeHeight,ll.Objects[0].height
                rootDate=bt.decimalDate(ref.name.split('|')[-1])-ref.height
                for k in ll.Objects:
                    k.absoluteTime=rootDate+k.height

                for target in target_tips:
                    # sys.stderr.write('%s %s %s\n'%(target.name,target.index,target.absoluteTime))
                    cur_node=target
                    path_to_root=[]
                    ...
                    ancestral_locs=[]
                    # print target in path_to_root
                    for t,tp in enumerate(timeline):
                        if rootDate<tp<rootDate+ll.treeHeight:
                            # branch=[k for k in path_to_root if k.parent.absoluteTime<tp<=k.absoluteTime]
                            branch=[k for k in path_to_root if tp<=k.absoluteTime and rootDate<tp]

                            #print tp,len(branch),target.absoluteTime,bt.decimalDate(target.name.split('|')[-1]),target.name
                            if len(branch)>0:
                                branch=branch[-1]
                                ancestral_locs.append(locations.index(branch.traits['location.states']))
                            else:
                                ancestral_locs.append('None')
                        else:
                            ancestral_locs.append('None')
                    # print target.name,target.traits['location.states'],[w for w in ancestral_locs if w!='None'][-1]
                    # print '\t%s'%('\t'.join(map(str,ancestral_locs)))
                    outfile.write('\t%s'%('\t'.join(map(str,ancestral_locs))))
